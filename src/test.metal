...
#include <simd/simd.h>
#include "/Users/lewislee/Documents/Coding/3d_renderer/include/RayTracingStructs.hpp"
...
Vec3 sampleSquare(Vec3 v) {
    ...
    float fr1 = (1 - 0) * (r1 * 2.3283064365387e-10);
    float fr2 = (1 - 0) * (r2 * 2.3283064365387e-10);
    return (Vec3){fr1 - 0.5f, fr2 - 0.5f, 0};
}
...
bool scatterDielectric(const Material mat, const Ray rIn, const HitRecord rec, thread Vec3* attenuation, thread Ray* scattered) {
    *attenuation = (Vec3){1.0, 1.0, 1.0};
    ...
    float cosTheta = metal::fmin(dot(unitDirection * -1, rec.normal), 1.0);
...
Ray getRay(int x, int y, const struct CameraData camData) {
    Vec3 offset = sampleSquare((Vec3){x + camData.center.z, y + camData.center.y, camData.center.x});
